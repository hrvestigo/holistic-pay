...
*/}}
{{- define "sirius-posting.chart" -}}
{{- printf "%s-%s" .Chart.Name .Chart.Version | replace "+" "_" | trunc 63 | trimSuffix "-" }}
{{- end }}

{{/*
Defies fixed part of sirius-posting datasource schema name
*/}}
{{- define "sirius-posting.dbSchema" -}}
{{- "sirpos" }}
{{- end }}

{{/*
sirius-posting image repository
*/}}
{{- define "sirius-posting.app.repository" -}}
{{- if .Values.image.app.imageLocation }}
{{- .Values.image.app.imageLocation }}
{{- else }}
{{- $psRepo := "hrvestigo/sirius-posting-ms" }}
...
{{/*
sirius-posting image pull policy
*/}}
{{- define "sirius-posting.app.imagePullPolicy" -}}
...
*/}}
{{- define "sirius-posting.liquibase.initContainer" }}
{{- range $k, $member := .Values.members }}
- name: liquibase-{{ .memberSign | lower }}
  securityContext:
  {{- toYaml $.Values.securityContext | nindent 4 }}
  image: {{ printf "%s%s%s%s%s" (include "sirius-posting.liquibase.image" $) "-" ($member.businessUnit | lower ) ":" $.Values.image.liquibase.tag }}
  imagePullPolicy: {{ default "IfNotPresent" (default $.Values.image.pullPolicy $.Values.image.liquibase.pullPolicy) }}
  resources:
    {{- include "sirius-posting.liquibase.initContainer.resources" $ | nindent 4 }}
  volumeMounts:
    - mountPath: /liquibase/secret/
      name: {{ include "sirius-posting.name" $ }}-secret
  env:
    - name: SCHEMA_NAME
  {{- if $member.datasource }}
    {{- if $member.datasource.globalSchema }}
      value: {{ $member.businessUnit | lower }}{{ required "Please specify global schema prefix in datasource.globalSchemaPrefix" $.Values.datasource.globalSchemaPrefix }}{{ include "sirius-posting.dbSchema" $ }}{{ required "Please specify environment label in env.label" $.Values.env.label | lower }}
    {{- else }}
      value: {{ $member.businessUnit | lower }}{{ $member.applicationMember | lower }}{{ include "sirius-posting.dbSchema" $ }}{{ required "Please specify environment label in env.label" $.Values.env.label | lower }}
    {{- end }}
  {{- else }}
      value: {{ $member.businessUnit | lower }}{{ $member.applicationMember | lower }}{{ include "sirius-posting.dbSchema" $ }}{{ required "Please specify environment label in env.label" $.Values.env.label | lower }}
  {{- end }}
...
*/}}
{{- define "sirius-posting.liquibase.initContainer.resources" -}}
...
*/}}
{{- define "sirius-posting.trustStoreEnv" -}}
...
Common labels
*/}}
{{- define "sirius-posting.labels" -}}
helm.sh/chart: {{ include "sirius-posting.chart" . }}
app: {{ include "sirius-posting.name" . }}
project: HolisticPay
{{ include "sirius-posting.selectorLabels" . }}
{{- if .Chart.AppVersion }}
...
Selector labels
*/}}
{{- define "sirius-posting.selectorLabels" -}}
app.kubernetes.io/name: {{ include "sirius-posting.name" . }}
...
- name: {{ include "sirius-posting.name" . }}-secret
  secret:
    secretName: {{ include "sirius-posting.name" . }}-secret
    items:
      - path: password.conf
        key: password.conf
- name: {{ include "sirius-posting.name" . }}-configmap
  configMap:
    name: {{ include "sirius-posting.name" . }}-configmap
- name: server-cert
...
{{/*
Mounts for sirius-posting application
*/}}
{{- define "sirius-posting.mounts" -}}
...
Application secrets
*/}}
{{- define "sirius-posting.passwords" -}}
...
*/}}
{{- define "sirius-posting.db.connectionParams" -}}
{{- $atts := list -}}
...
Kafka login module
*/}}
{{- define "sirius-posting.kafka.loginModule" -}}
