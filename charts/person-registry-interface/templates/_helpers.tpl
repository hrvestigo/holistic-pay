...
*/}}
{{- define "person-registry-interface.name" -}}
...
*/}}
{{- define "person-registry-interface.chart" -}}
{{- printf "%s-%s" .Chart.Name .Chart.Version | replace "+" "_" | trunc 63 | trimSuffix "-" }}
{{- end }}

{{/*
Defies fixed part of person-registry-interface datasource schema name
*/}}
{{- define "person-registry-interface.dbSchema" -}}
{{- "preint" }}
{{- end }}

{{/*
person-registry-interface image repository
*/}}
{{- define "person-registry-interface.app.repository" -}}
{{- if .Values.image.app.imageLocation }}
{{- .Values.image.app.imageLocation }}
{{- else }}
{{- $psRepo := "hrvestigo/person-registry-interface-ms" }}
...
{{/*
person-registry-interface image pull policy
*/}}
{{- define "person-registry-interface.app.imagePullPolicy" -}}
...
*/}}
{{- define "person-registry-interface.liquibase.initContainer" }}
{{- range $k, $member := .Values.members }}
- name: liquibase-{{ .memberSign | lower }}
  securityContext:
  {{- toYaml $.Values.securityContext | nindent 4 }}
  image: {{ printf "%s%s%s%s%s" (include "person-registry-interface.liquibase.image" $) "-" ($member.businessUnit | lower ) ":" $.Values.image.liquibase.tag }}
  imagePullPolicy: {{ default "IfNotPresent" (default $.Values.image.pullPolicy $.Values.image.liquibase.pullPolicy) }}
  resources:
    {{- include "person-registry-interface.liquibase.initContainer.resources" $ | nindent 4 }}
  volumeMounts:
    - mountPath: /liquibase/secret/
      name: {{ include "person-registry-interface.name" $ }}-secret
  env:
    - name: SCHEMA_NAME
  {{- if $member.datasource }}
    {{- if $member.datasource.globalSchema }}
      value: {{ $member.businessUnit | lower }}{{ required "Please specify global schema prefix in datasource.globalSchemaPrefix" $.Values.datasource.globalSchemaPrefix }}{{ include "person-registry-interface.dbSchema" $ }}{{ required "Please specify environment label in env.label" $.Values.env.label | lower }}
    {{- else }}
      value: {{ $member.businessUnit | lower }}{{ $member.applicationMember | lower }}{{ include "person-registry-interface.dbSchema" $ }}{{ required "Please specify environment label in env.label" $.Values.env.label | lower }}
    {{- end }}
  {{- else }}
      value: {{ $member.businessUnit | lower }}{{ $member.applicationMember | lower }}{{ include "person-registry-interface.dbSchema" $ }}{{ required "Please specify environment label in env.label" $.Values.env.label | lower }}
  {{- end }}
...
*/}}
{{- define "person-registry-interface.liquibase.initContainer.resources" -}}
...
*/}}
{{- define "person-registry-interface.trustStoreEnv" -}}
...
Common labels
*/}}
{{- define "person-registry-interface.labels" -}}
helm.sh/chart: {{ include "person-registry-interface.chart" . }}
app: {{ include "person-registry-interface.name" . }}
project: HolisticPay
{{ include "person-registry-interface.selectorLabels" . }}
{{- if .Chart.AppVersion }}
...
Selector labels
*/}}
{{- define "person-registry-interface.selectorLabels" -}}
app.kubernetes.io/name: {{ include "person-registry-interface.name" . }}
...
Volumes
*/}}
{{- define "person-registry-interface.volumes" -}}
...
- name: {{ include "person-registry-interface.name" . }}-secret
  secret:
    secretName: {{ include "person-registry-interface.name" . }}-secret
    items:
      - path: password.conf
        key: password.conf
- name: {{ include "person-registry-interface.name" . }}-configmap
  configMap:
    name: {{ include "person-registry-interface.name" . }}-configmap
- name: server-cert
...
{{/*
Mounts for person-registry-interface application
*/}}
{{- define "person-registry-interface.mounts" -}}
...
*/}}
{{- define "person-registry-interface.members" -}}
{{- $members := list -}}
{{- range $k, $member := .Values.members }}
...
Application secrets
*/}}
{{- define "person-registry-interface.passwords" -}}
...
Application logger
*/}}
{{- define "person-registry-interface.logger" -}}
...
*/}}
{{- define "person-registry-interface.db.connectionParams" -}}
{{- $atts := list -}}
...
Kafka login module
*/}}
{{- define "person-registry-interface.kafka.loginModule" -}}
