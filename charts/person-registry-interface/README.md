# person-registry-interface application

## Purpose

This Helm chart installs person-registry-interface application into your Kubernetes cluster.
...
It is not possible to install application using default values only, there is a list of required attributes which should be applied when installing person-registry-interface.
...
`helm upgrade --install person-registry-interface holisticpay/person-registry-interface -f my-values.yaml` or

`helm upgrade --install person-registry-interface holisticpay/person-registry-interface --set required-key1=value1 --set required-key2=value2 ...`
...
person-registry-interface application relies on Kafka and PostgreSQL backends.
...
person-registry-interface  uses Kafka as event stream backend.
Other than Kafka cluster itself,  person-registry-interface e application also uses Kafka Schema Registry, which setup also has to be provided in order to establish required connection.
...
Default Kafka cluster and Kafka Schema registry connection type used by person-registry-interface is Basic auth (username and password).
...
Kafka topics and consumer group names used by person-registry-interface have default names defined in `values.yaml` file, but can be overridden with following setup:

```yaml
kafka:
  topics:
    confirmpersoncreation:
      name: hr.vestigo.hp.confirmpersoncreation
...
By default,  person-registry-interface  image is pulled directly from Vestigo's repository hosted by Docker Hub.
...
image:
  ...
  app:
    pullPolicy: Never # will override image.pullPolicy for person-registry-interface image
  liquibase:
    pullPolicy: IfNotPresent # will override image.pullPolicy for Liquibase image
```

person-registry-interface  image tag is normally read from Chart definition, but if required, it can be overridden with attribute `image.app.tag`, for example:
...
person-registry-interface  image is located on Vestigo's private Docker Hub registry, and if image registry is set to docker.io, pull secret has to be defined.
Pull secret is not set by default, and it should be created prior to person-registry-interface installation in target namespace.
...
person-registry-interface  application is prepared to use TLS, but requires provided server certificate.
...
**Note that this secret has to be created in target namespace prior to installation of person-registry-interface application.**
...
```yaml
customVolumes:
  - name: trust-store-volume-name # has to match name in initContainer and volumeMount in person-registry-interface container
    emptyDir: # any other volume type is OK
      medium: "Memory"
```

person-registry-interface container should also mount this volume, so a custom `volumeMount` is required, for example:
...
Note that `mountPath` variable is used to specify a location of trust store in person-registry-interface container.
...
mTLS support can be added to person-registry-interface application in two different ways.
...
```yaml
customVolumes:
  - name: key-store-volume-name # has to match name in initContainer and volumeMount in person-registry-interface container
    emptyDir: # any other volume type is OK
      medium: "Memory"
```
...
Note that `mountPath` variable is used to specify a location of key store in person-registry-interface container.
Suggested location is: `/mnt/k8s/trust-store`.
...
Besides required attributes, installation of person-registry-interface can be customized in different ways.

### Multi-member setup

person-registry-interface  application (along with all other HolisticPay applications) supports multi-member setup. In order to complete application setup, at least a mandatory set of attributes has to be defined:
...
person-registry-interface application can use oAuth2 service for authorization. By default, this option is disabled, but can easily be enabled by specifying following attributes in values:
...
Custom environment variables can be added to person-registry-interface container by applying `customEnv` value, for example:
...
Values file can be used to specify additional custom `volume` and `volumeMounts` to be added to person-registry-interface container.

For example, custom volume mount could be added by defining this setup:

```yaml
customVolumes:
  - name: my-custom-volume # has to match name in initContainer and volumeMount in person-registry-interface container
    emptyDir: # any other volume type is OK
      medium: "Memory"
...
person-registry-interface application is predefined to redirect all logs to `stdout` expect for Web Server logs (`access.log`) and health check logs, which are not logged by default.
...
Default deployment strategy for person-registry-interface application is `RollingUpdate`, but it can be overridden, along with other deployment parameters using following attributes (default values are shown):
...
By default, one replica of person-registry-interface is installed on Kubernetes cluster. Number of replicas can be statically modified with above configuration, or `HorizontalPodAutoscaler` option can be used to let Kubernetes automatically scale application when required.

#### Customizing pod resource requests and limits

Following are the default values for person-registry-interface requests and limits:
...
CPU and/or memory utilization metrics can be used to autoscale person-registry-interface pod.
...
person-registry-interface application has predefined health check probes (readiness and liveness).
...
Note that  person-registry-interface  has health checks available within the `/health` endpoint (`/health/readiness` for readiness and `/health/liveness` for liveness), and this base paths should not modified, only query parameters are subject to change.
...
Security context for person-registry-interface can be set on pod and/or on container level.
...
Note that container level security context will be applied to both containers in person-registry-interface pod (Liquibase init container and person-registry-interface container).

### Customizing network setup

#### Service setup

When installing person-registry-interface using default setup, a `Service` object will be created of `ClusterIP` type exposed on port 8443.
...
ingress:
  ...
  hosts:
    - host: person-registry-interface.custom.url
      paths:
        - path: /
          pathType: Prefix
```
...
person-registry-interface deployment has some predefined affinity rules, as listed below:
...
There are some other customizable attributes predefined in  person-registry-interface  application.
...
Finally, since  person-registry-interface  is an Java application, there's a possibility to set custom JVM parameters.
