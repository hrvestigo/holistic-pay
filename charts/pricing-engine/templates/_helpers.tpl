...
*/}}
{{- define "pricing-engine.chart" -}}
{{- printf "%s-%s" .Chart.Name .Chart.Version | replace "+" "_" | trunc 63 | trimSuffix "-" }}
{{- end }}

{{/*
Defies fixed part of pricing-engine datasource schema name
*/}}
{{- define "pricing-engine.dbSchema" -}}
{{- "prceng" }}
{{- end }}

{{/*
pricing-engine image repository
*/}}
{{- define "pricing-engine.app.repository" -}}
{{- if .Values.image.app.imageLocation }}
{{- .Values.image.app.imageLocation }}
{{- else }}
{{- $psRepo := "hrvestigo/pricing-engine-ms" }}
...
{{/*
pricing-engine image pull policy
*/}}
{{- define "pricing-engine.app.imagePullPolicy" -}}
...
*/}}
{{- define "pricing-engine.liquibase.initContainer" }}
{{- range $k, $member := .Values.members }}
- name: liquibase-{{ .memberSign | lower }}
  securityContext:
  {{- toYaml $.Values.securityContext | nindent 4 }}
  {{- if $.Values.image.liquibase.imageLocation }}
  image: {{ include "pricing-engine.liquibase.image" $ }}
  {{- else }}
  image: {{ printf "%s%s%s%s%s" (include "pricing-engine.liquibase.image" $) "-" ($member.memberSign | lower) ":" $.Values.image.liquibase.tag }}
  {{- end }}
  imagePullPolicy: {{ default "IfNotPresent" (default $.Values.image.pullPolicy $.Values.image.liquibase.pullPolicy) }}
  resources:
    {{- include "pricing-engine.liquibase.initContainer.resources" $ | nindent 4 }}
  volumeMounts:
  ...
  {{- if $member.datasource }}
    {{- if $member.datasource.globalSchema }}
      value: {{ $member.businessUnit | lower }}{{ required "Please specify global schema prefix in datasource.globalSchemaPrefix" $.Values.datasource.globalSchemaPrefix }}{{ include "pricing-engine.dbSchema" $ }}{{ required "Please specify environment label in env.label" $.Values.env.label | lower }}
    {{- else }}
      value: {{ $member.businessUnit | lower }}{{ $member.applicationMember | lower }}{{ include "pricing-engine.dbSchema" $ }}{{ required "Please specify environment label in env.label" $.Values.env.label | lower }}
    {{- end }}
  {{- else }}
      value: {{ $member.businessUnit | lower }}{{ $member.applicationMember | lower }}{{ include "pricing-engine.dbSchema" $ }}{{ required "Please specify environment label in env.label" $.Values.env.label | lower }}
  {{- end }}
...
*/}}
{{- define "pricing-engine.liquibase.initContainer.resources" -}}
...
*/}}
{{- define "pricing-engine.trustStoreEnv" -}}
...
helm.sh/chart: {{ include "pricing-engine.chart" . }}
app: {{ include "pricing-engine.name" . }}
project: HolisticPay
{{ include "pricing-engine.selectorLabels" . }}
{{- if .Chart.AppVersion }}
...
- name: {{ include "pricing-engine.name" . }}-secret
  secret:
    ...
    items:
      - path: password.conf
        key: password.conf
- name: {{ include "pricing-engine.name" . }}-configmap
  configMap:
    name: {{ include "pricing-engine.name" . }}-configmap
- name: server-cert
...
{{/*
Mounts for pricing-engine application
*/}}
{{- define "pricing-engine.mounts" -}}
...
Application secrets
*/}}
{{- define "pricing-engine.passwords" -}}
...
*/}}
{{- define "pricing-engine.db.connectionParams" -}}
{{- $atts := list -}}
...
Kafka login module
*/}}
{{- define "pricing-engine.kafka.loginModule" -}}
