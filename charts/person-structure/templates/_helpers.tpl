...
- name: liquibase-{{ .memberSign | lower }}
  ...
  {{- else }}
  image: {{ printf "%s%s%s%s%s" (include "person-structure.liquibase.image" $) "-" ($member.memberSign | lower ) ":" $.Values.image.liquibase.tag }}
  {{- end }}
...
Kafka login module
*/}}
{{- define "person-structure.kafka.loginModule" -}}
