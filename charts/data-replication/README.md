# data-replication-ms application


This Helm chart installs data-replication-ms application into your Kubernetes cluster.
...
It is not possible to install application using default values only, there is a list of required attributes which should be applied when installing data-replication-ms.
...
`helm upgrade --install data-replication-ms holisticpay/data-replication-ms -f my-values.yaml` or

`helm upgrade --install data-replication-ms holisticpay/data-replication-ms --set required-key1=value1 --set required-key2=value2 ...`
...
data-replication-ms application relies on Kafka and PostgreSQL backends.
...
data-replication-ms (as well as all other HolisticPay applications) is a multi-member application. For this reason, at least one application member has to be defined in `members` structure for complete setup. Please refer to [Multi-member setup](#multi-member-setup) for details.
...
Default Kafka cluster and Kafka Schema registry connection type used by data-replication-ms is Basic auth (username and password).
...
image:
  ...
  app:
    pullPolicy: Never # will override image.pullPolicy for data-replication-ms image
  liquibase:
    pullPolicy: IfNotPresent # will override image.pullPolicy for Liquibase image
```

data-replication-ms  image tag is normally read from Chart definition, but if required, it can be overridden with attribute `image.app.tag`, for example:
...
data-replication-ms  image is located on Vestigo's private Docker Hub registry, and if image registry is set to docker.io, pull secret has to be defined.
Pull secret is not set by default, and it should be created prior to data-replication-ms installation in target namespace.
...
data-replication-ms  application is prepared to use TLS, but requires provided server certificate.
...
**Note that this secret has to be created in target namespace prior to installation of data-replication-ms application.**
...
```yaml
customVolumes:
  - name: trust-store-volume-name # has to match name in initContainer and volumeMount in data-replication-ms container
    emptyDir: # any other volume type is OK
      medium: "Memory"
```

data-replication-ms container should also mount this volume, so a custom `volumeMount` is required, for example:
...
Note that `mountPath` variable is used to specify a location of trust store in data-replication-ms container.
...
mTLS support can be added to data-replication-ms application in two different ways.
...
```yaml
customVolumes:
  - name: key-store-volume-name # has to match name in initContainer and volumeMount in data-replication-ms container
    emptyDir: # any other volume type is OK
      medium: "Memory"
```
...
Note that `mountPath` variable is used to specify a location of key store in data-replication-ms container.
Suggested location is: `/mnt/k8s/trust-store`.
...
Besides required attributes, installation of data-replication-ms can be customized in different ways.


data-replication-ms  application (along with all other HolisticPay applications) supports multi-member setup. In order to complete application setup, at least a mandatory set of attributes has to be defined:
...
data-replication-ms application can use oAuth2 service for authorization. By default, this option is disabled, but can easily be enabled by specifying following attributes in values:
...
Custom environment variables can be added to data-replication-ms container by applying `customEnv` value, for example:
...
Values file can be used to specify additional custom `volume` and `volumeMounts` to be added to data-replication-ms container.

For example, custom volume mount could be added by defining this setup:

```yaml
customVolumes:
  - name: my-custom-volume # has to match name in initContainer and volumeMount in data-replication-ms container
    emptyDir: # any other volume type is OK
      medium: "Memory"
...
data-replication-ms application is predefined to redirect all logs to `stdout` expect for Web Server logs (`access.log`) and health check logs, which are not logged by default.
...
Default deployment strategy for data-replication-ms application is `RollingUpdate`, but it can be overridden, along with other deployment parameters using following attributes (default values are shown):
...
By default, one replica of data-replication-ms is installed on Kubernetes cluster. Number of replicas can be statically modified with above configuration, or `HorizontalPodAutoscaler` option can be used to let Kubernetes automatically scale application when required.


Following are the default values for data-replication-ms requests and limits:
...
CPU and/or memory utilization metrics can be used to autoscale data-replication-ms pod.
...
Note that  data-replication-ms  has health checks available within the `/health` endpoint (`/health/readiness` for readiness and `/health/liveness` for liveness), and this base paths should not modified, only query parameters are subject to change.
`scheme` attribute should also be set to `HTTPS` at all times, as well as `http` value for `port` attribute.


Security context for data-replication-ms can be set on pod and/or on container level.
...
Note that container level security context will be applied to both containers in data-replication-ms pod (Liquibase init container and data-replication-ms container).



When installing data-replication-ms using default setup, a `Service` object will be created of `ClusterIP` type exposed on port 8443.
...
ingress:
  ...
  hosts:
    - host: data-replication-ms.custom.url
      paths:
        - path: /
          pathType: Prefix
```
...
data-replication-ms deployment has some predefined affinity rules, as listed below:
...
Finally, since  data-replication-ms  is an Java application, there's a possibility to set custom JVM parameters.
