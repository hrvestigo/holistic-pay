...
*/}}
{{- define "data-replication-ms.name" -}}
...
*/}}
{{- define "data-replication-ms.chart" -}}
{{- printf "%s-%s" .Chart.Name .Chart.Version | replace "+" "_" | trunc 63 | trimSuffix "-" }}
{{- end }}

{{/*
Defies fixed part of data-replication-ms datasource schema name
*/}}
{{- define "data-replication-ms.dbSchema" -}}
{{- "datrep" }}
{{- end }}

{{/*
data-replication-ms image repository
*/}}
{{- define "data-replication-ms.app.repository" -}}
...
{{/*
data-replication-ms image pull policy
*/}}
{{- define "data-replication-ms.app.imagePullPolicy" -}}
...
- name: liquibase-{{ $member.memberSign | lower }}
  ...
  {{- if $member.datasource }}
    ...
    {{- end }}
  {{- else }}
    {{- $url := printf "%s%s%s%d%s%s" "jdbc:postgresql://" $.Values.datasource.target.host ":" ($.Values.datasource.target.port | int) "/" $.Values.datasource.target.dbName }}
    {{- $context := printf "%s%s%s" (required "Please specify business unit in members.businessUnit" $member.businessUnit | upper) (required "Please specify application member in members.applicationMember" $member.applicationMember | upper) ",test" }}
    {{- $params := printf "%s%s%s%s%s%s" "cp /liquibase/changelog/liquibase.properties /tmp && java -jar /tmp/aesdecryptor.jar -d -l && /liquibase/docker-entrypoint.sh --defaultsFile=/tmp/liquibase.properties --url=" $url " --contexts=" $context " --username=" $.Values.liquibase.user }}
    {{- if $.Values.liquibase.syncOnly }}
    - {{ printf "%s%s" $params " changelog-sync" }}
...
*/}}
{{- define "data-replication-ms.liquibase.initContainer.resources" -}}
...
*/}}
{{- define "data-replication-ms.trustStoreEnv" -}}
...
Common labels
*/}}
{{- define "data-replication-ms.labels" -}}
helm.sh/chart: {{ include "data-replication-ms.chart" . }}
app: {{ include "data-replication-ms.name" . }}
project: HolisticPay
{{ include "data-replication-ms.selectorLabels" . }}
{{- if .Chart.AppVersion }}
...
Selector labels
*/}}
{{- define "data-replication-ms.selectorLabels" -}}
app.kubernetes.io/name: {{ include "data-replication-ms.name" . }}
...
Volumes
*/}}
{{- define "data-replication-ms.volumes" -}}
...
{{/*
Mounts for data-replication-ms application
*/}}
{{- define "data-replication-ms.mounts" -}}
...
*/}}
{{- define "data-replication-ms.members" -}}
{{- $members := list -}}
{{- range $k, $member := .Values.members }}
...
Application secrets
*/}}
{{- define "data-replication-ms.passwords" -}}
...
Application logger
*/}}
{{- define "data-replication-ms.logger" -}}
...
*/}}
{{- define "data-replication-ms.db.connectionParams" -}}
{{- $atts := list -}}
...
Kafka login module
*/}}
{{- define "data-replication.kafka.loginModule" -}}
