...
This Helm chart installs Product factory application into your Kubernetes cluster.
...
It is not possible to install application using default values only, there is a list of required attributes which should be applied when installing Product factory.
...
`helm upgrade --install product-factory holisticpay/product-factory -f my-values.yaml` or

`helm upgrade --install product-factory holisticpay/product-factory --set required-key1=value1 --set required-key2=value2 ...`
...
Product factory uses Kafka as event stream backend.
Other than Kafka cluster itself, Product factory application also uses Kafka Schema Registry, which setup also has to be provided in order to establish required connection.
...
Default Kafka cluster and Kafka Schema registry connection type used by Product factory is Basic auth (username and password).
...
kafka:
  topics:
    ...
    cobaffschlimit:
      name: hr.vestigo.hp.cobaffschlimit # default value, set custom name if required
      consumerGroup: hr.vestigo.hp.cobaffschlimit # default value, set custom name if required
    financiallimittype:
...
image:
  ...
  app:
    registry: custom.app.image.registry # will override image.registry for Product factory app
  liquibase:
...
image:
  ...
  app:
    pullPolicy: Never # will override image.pullPolicy for Product factory image
  liquibase:
    pullPolicy: IfNotPresent # will override image.pullPolicy for Liquibase image
```

Product factory image tag is normally read from Chart definition, but if required, it can be overridden with attribute `image.app.tag`, for example:
...
Product factory image is located on Vestigo's private Docker Hub registry, and if image registry is set to docker.io, pull secret has to be defined.
Pull secret is not set by default, and it should be created prior to Product factory installation in target namespace.
...
Product factory application is prepared to use TLS, but requires provided server certificate.
...
**Note that this secret has to be created in target namespace prior to installation of Product factory application.**
...
```yaml
customVolumes:
  - name: trust-store-volume-name # has to match name in initContainer and volumeMount in product-factory container
    emptyDir: # any other volume type is OK
      medium: "Memory"
```

product-factory container should also mount this volume, so a custom `volumeMount` is required, for example:
...
Note that `mountPath` variable is used to specify a location of trust store in product-factory container.
...
mTLS support can be added to product-factory application in two different ways.
...
```yaml
customVolumes:
  - name: key-store-volume-name # has to match name in initContainer and volumeMount in product-factory container
    emptyDir: # any other volume type is OK
      medium: "Memory"
```
...
Note that `mountPath` variable is used to specify a location of key store in product-factory container.
Suggested location is: `/mnt/k8s/trust-store`.
...
Values file can be used to specify additional custom `volume` and `volumeMounts` to be added to product-factory container.

For example, custom volume mount could be added by defining this setup:

```yaml
customVolumes:
  - name: my-custom-volume # has to match name in initContainer and volumeMount in product-factory container
    emptyDir: # any other volume type is OK
      medium: "Memory"
...
Product factory application is predefined to redirect all logs to `stdout` expect for Web Server logs (`access.log`) and health check logs, which are not logged by default.
...
By default, one replica of Product factory is installed on Kubernetes cluster. Number of replicas can be statically modified with above configuration, or `HorizontalPodAutoscaler` option can be used to let Kubernetes automatically scale application when required.

#### Customizing pod resource requests and limits

Following are the default values for Product factory requests and limits:
...
CPU and/or memory utilization metrics can be used to autoscale Product factory pod.
...
Product factory application has predefined health check probes (readiness and liveness).
...
Note that Product factory has health checks available within the `/health` endpoint (`/health/readiness` for readiness and `/health/liveness` for liveness), and this base paths should not modified, only query parameters are subject to change.
...
Security context for Product factory can be set on pod and/or on container level.
...
Note that container level security context will be applied to both containers in Product factory pod (Liquibase init container and Product factory container).

### Customizing network setup

#### Service setup

When installing Product factory using default setup, a `Service` object will be created of `ClusterIP` type exposed on port 8443.
...
ingress:
  ...
  hosts:
    - host: product-factory.custom.url
      paths:
        - path: /
          pathType: Prefix
```
...
Product factory deployment has some predefined affinity rules, as listed below:
...
There are some other customizable attributes predefined in Product factory application.
...
Finally, since Product factory is an Java application, there's a possibility to set custom JVM parameters.
