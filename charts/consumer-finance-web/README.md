# Consumer finance web application

## Purpose

This Helm chart installs Consumer finance web application into your Kubernetes cluster.
...
It is not possible to install application using default values only, there is a list of required attributes which should be applied when installing Consumer finance web.
...
`helm upgrade --install consumer-finance-web holisticpay/consumer-finance-web -f my-values.yaml` or

`helm upgrade --install consumer-finance-web holisticpay/consumer-finance-web --set required-key1=value1 --set required-key2=value2 ...`
...
Consumer finance web applications requires a location (endpoint) of API Gateway through which it will be called (defined with `apiGatewayUrl` attribute).

### TLS setup

Consumer finance web application is prepared to use TLS, but requires provided server certificate.
...
**Note that this secret has to be created in target namespace prior to installation of Consumer finance web application.**
...
Besides required attributes, installation of Consumer finance web application can be customized in different ways.

### oAuth2

Consumer finance web application can use oAuth2 service for authorization. By default, this option is disabled, but can easily be enabled by specifying following attributes in values:
...
Custom environment variables can be added to consumer-finance-web container by applying `customEnv` value, for example:
...
Values file can be used to specify additional custom `volume` and `volumeMounts` to be added to consumer-finance-web container.

For example, custom volume mount could be added by defining this setup:

```yaml
customVolumes:
  - name: my-custom-volume # has to match name in initContainer and volumeMount in consumer-finance-web container
...
Default deployment strategy for Consumer finance web application is `RollingUpdate`, but it can be overridden, along with other deployment parameters using following attributes (default values are shown):
...
By default, one replica of Consumer finance web is installed on Kubernetes cluster. Number of replicas can be statically modified with above configuration, or `HorizontalPodAutoscaler` option can be used to let Kubernetes automatically scale application when required.

#### Customizing pod resource requests and limits

Following are the default values for Consumer finance web requests and limits:
...
CPU and/or memory utilization metrics can be used to autoscale Consumer finance web pod.
...
Consumer finance web application has predefined health check probes (readiness and liveness).
...
Note that Consumer finance web has health checks available within the `/health` endpoint (`/health/readiness` for readiness and `/health/liveness` for liveness), and this base paths should not modified, only query parameters are subject to change.
...
Security context for Consumer finance web can be set on pod and/or on container level.
...
When installing Consumer finance web using default setup, a `Service` object will be created of `ClusterIP` type exposed on port 8443.
...
ingress:
  ...
  hosts:
    - host: consumer-finance-web.custom.url
      paths:
        - path: /
          pathType: Prefix
```
...
Consumer finance web deployment has some predefined affinity rules, as listed below:
...
There are some other customizable attributes predefined in Consumer finance web application.
