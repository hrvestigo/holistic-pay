...
{{/*
Defies fixed part of mocks datasource schema name
*/}}
{{- define "mocks.dbSchema" -}}
{{- "mocks" }}
{{- end }}
...
{{/*
mocks image pull policy
*/}}
{{- define "mocks.app.imagePullPolicy" -}}
...
*/}}
{{- define "mocks.liquibase.initContainer" }}
{{- range $k, $member := .Values.members }}
- name: liquibase-{{ .memberSign | lower }}
  securityContext:
  {{- toYaml $.Values.securityContext | nindent 4 }}
  {{- if $.Values.image.liquibase.imageLocation }}
  image: {{ include "mocks.liquibase.image" $ }}
  {{- else }}
  image: {{ printf "%s%s%s%s%s" (include "mocks.liquibase.image" $) "-" ($member.businessUnit | lower ) ":" $.Values.image.liquibase.tag }}
  {{- end }}
  imagePullPolicy: {{ default "IfNotPresent" (default $.Values.image.pullPolicy $.Values.image.liquibase.pullPolicy) }}
  resources:
    {{- include "mocks.liquibase.initContainer.resources" $ | nindent 4 }}
  volumeMounts:
  ...
  {{- if $member.datasource }}
    {{- if $member.datasource.globalSchema }}
      value: {{ $member.businessUnit | lower }}{{ required "Please specify global schema prefix in datasource.globalSchemaPrefix" $.Values.datasource.globalSchemaPrefix }}{{ include "mocks.dbSchema" $ }}{{ required "Please specify environment label in env.label" $.Values.env.label | lower }}
    {{- else }}
      value: {{ $member.businessUnit | lower }}{{ $member.applicationMember | lower }}{{ include "mocks.dbSchema" $ }}{{ required "Please specify environment label in env.label" $.Values.env.label | lower }}
    {{- end }}
  {{- else }}
      value: {{ $member.businessUnit | lower }}{{ $member.applicationMember | lower }}{{ include "mocks.dbSchema" $ }}{{ required "Please specify environment label in env.label" $.Values.env.label | lower }}
  {{- end }}
...
*/}}
{{- define "mocks.liquibase.initContainer.resources" -}}
...
*/}}
{{- define "mocks.labels" -}}
helm.sh/chart: {{ include "mocks.chart" . }}
app: {{ include "mocks.name" . }}
project: HolisticPay
{{ include "mocks.selectorLabels" . }}
{{- if .Chart.AppVersion }}
...
Selector labels
*/}}
{{- define "mocks.selectorLabels" -}}
app.kubernetes.io/name: {{ include "mocks.name" . }}
...
- name: {{ include "mocks.name" . }}-secret
  secret:
    ...
    items:
      - path: password.conf
        key: password.conf
- name: {{ include "mocks.name" . }}-configmap
  configMap:
    name: {{ include "mocks.name" . }}-configmap
- name: server-cert
...
{{/*
Mocks Kafka topics configuration
*/}}
{{- define "mocks.kafka.topics" }}
{{- range $key, $value := .Values.kafka.topics }}
{{- range $k, $v := $value }}
- name: MOCKS_KAFKA_TOPICS_{{ $key | upper }}_{{ $k | snakecase | upper }}
  value: {{ $v | quote }}
{{- end }}
{{- end }}
{{- end }}
...
Final configuration name is MOCKS_<module>_<moduleKey>_<name>=<value>,
where 'name' is configuration parameter and 'value' is configuration value
*/}}
{{- define "mocks.application.config" }}
{{- range $module, $configs := .Values.application }}
{{- range $moduleKey, $config := $configs }}
{{- range $name, $value := $config }}
- name: MOCKS_{{ $module | upper }}_{{ $moduleKey | snakecase | upper }}_{{ $name | snakecase | upper }}
  value: {{ $value | quote }}
{{- end }}
{{- end }}
{{- end }}
...
*/}}
{{- define "mocks.db.connectionParams" -}}
{{- $atts := list -}}
...
*/}}
{{- define "mocks.exposed.endpoints" -}}
{{- $endpoints := list -}}
