...
*/}}
{{- define "ecs-interface.chart" -}}
{{- printf "%s-%s" .Chart.Name .Chart.Version | replace "+" "_" | trunc 63 | trimSuffix "-" }}
{{- end }}

{{/*
Defies fixed part of ecs-interface datasource schema name
*/}}
{{- define "ecs-interface.dbSchema" -}}
{{- "ecsint" }}
{{- end }}

{{/*
ecs-interface image repository
*/}}
{{- define "ecs-interface.app.repository" -}}
{{- if .Values.image.app.imageLocation }}
{{- .Values.image.app.imageLocation }}
{{- else }}
{{- $psRepo := "hrvestigo/ecs-interface-ms" }}
...
{{/*
ecs-interface image pull policy
*/}}
{{- define "ecs-interface.app.imagePullPolicy" -}}
...
*/}}
{{- define "ecs-interface.liquibase.initContainer" }}
{{- range $k, $member := .Values.members }}
- name: liquibase-{{ .memberSign | lower }}
  securityContext:
  {{- toYaml $.Values.securityContext | nindent 4 }}
  image: {{ printf "%s%s%s%s%s" (include "ecs-interface.liquibase.image" $) "-" ($member.businessUnit | lower ) ":" $.Values.image.liquibase.tag }}
  imagePullPolicy: {{ default "IfNotPresent" (default $.Values.image.pullPolicy $.Values.image.liquibase.pullPolicy) }}
  resources:
    {{- include "ecs-interface.liquibase.initContainer.resources" $ | nindent 4 }}
  volumeMounts:
    - mountPath: /liquibase/secret/
      name: {{ include "ecs-interface.name" $ }}-secret
  env:
    - name: SCHEMA_NAME
  {{- if $member.datasource }}
    {{- if $member.datasource.globalSchema }}
      value: {{ $member.businessUnit | lower }}{{ required "Please specify global schema prefix in datasource.globalSchemaPrefix" $.Values.datasource.globalSchemaPrefix }}{{ include "ecs-interface.dbSchema" $ }}{{ required "Please specify environment label in env.label" $.Values.env.label | lower }}
    {{- else }}
      value: {{ $member.businessUnit | lower }}{{ $member.applicationMember | lower }}{{ include "ecs-interface.dbSchema" $ }}{{ required "Please specify environment label in env.label" $.Values.env.label | lower }}
    {{- end }}
  {{- else }}
      value: {{ $member.businessUnit | lower }}{{ $member.applicationMember | lower }}{{ include "ecs-interface.dbSchema" $ }}{{ required "Please specify environment label in env.label" $.Values.env.label | lower }}
  {{- end }}
...
*/}}
{{- define "ecs-interface.liquibase.initContainer.resources" -}}
...
Kafka login module
*/}}
{{- define "ecs-interface.kafka.loginModule" -}}
...
*/}}
{{- define "ecs-interface.trustStoreEnv" -}}
...
Common labels
*/}}
{{- define "ecs-interface.labels" -}}
helm.sh/chart: {{ include "ecs-interface.chart" . }}
app: {{ include "ecs-interface.name" . }}
project: HolisticPay
{{ include "ecs-interface.selectorLabels" . }}
{{- if .Chart.AppVersion }}
...
Selector labels
*/}}
{{- define "ecs-interface.selectorLabels" -}}
app.kubernetes.io/name: {{ include "ecs-interface.name" . }}
...
- name: {{ include "ecs-interface.name" . }}-secret
  secret:
    secretName: {{ include "ecs-interface.name" . }}-secret
    items:
      - path: password.conf
        key: password.conf
- name: {{ include "ecs-interface.name" . }}-configmap
  configMap:
    name: {{ include "ecs-interface.name" . }}-configmap
- name: server-cert
...
{{/*
Mounts for ecs-interface application
*/}}
{{- define "ecs-interface.mounts" -}}
...
Application secrets
*/}}
{{- define "ecs-interface.passwords" -}}
...
*/}}
{{- define "ecs-interface.db.connectionParams" -}}
{{- $atts := list -}}
...
*/}}
{{- define "ecs-interface.exposed.endpoints" -}}
{{- $endpoints := list -}}
