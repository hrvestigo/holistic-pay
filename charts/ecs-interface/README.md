# ecs-interface application

## Purpose

This Helm chart installs ecs-interface application into your Kubernetes cluster.
...
It is not possible to install application using default values only, there is a list of required attributes which should be applied when installing ecs-interface.
...
`helm upgrade --install ecs-interface holisticpay/ecs-interface -f my-values.yaml` or

`helm upgrade --install ecs-interface holisticpay/ecs-interface --set required-key1=value1 --set required-key2=value2 ...`
...
ecs-interface application relies on Kafka and PostgreSQL backends.
...
ecs-interface (as well as all other HolisticPay applications) is a multi-member application. For this reason, at least one application member has to be defined in `members` structure for complete setup. Please refer to [Multi-member setup](#multi-member-setup) for details.
...
ecs-interface  uses Kafka as event stream backend.
Other than Kafka cluster itself,  ecs-interface e application also uses Kafka Schema Registry, which setup also has to be provided in order to establish required connection.
...
Default Kafka cluster and Kafka Schema registry connection type used by ecs-interface is Basic auth (username and password).
...
Kafka topics and consumer group names used by ecs-interface have default names defined in `values.yaml` file, but can be overridden with following setup:
...
image:
  ...
  app:
    pullPolicy: Never # will override image.pullPolicy for ecs-interface image
  liquibase:
    pullPolicy: IfNotPresent # will override image.pullPolicy for Liquibase image
```

ecs-interface  image tag is normally read from Chart definition, but if required, it can be overridden with attribute `image.app.tag`, for example:
...
ecs-interface  image is located on Vestigo's private Docker Hub registry, and if image registry is set to docker.io, pull secret has to be defined.
Pull secret is not set by default, and it should be created prior to ecs-interface installation in target namespace.
...
ecs-interface  application is prepared to use TLS, but requires provided server certificate.
...
**Note that this secret has to be created in target namespace prior to installation of ecs-interface application.**
...
```yaml
customVolumes:
  - name: trust-store-volume-name # has to match name in initContainer and volumeMount in ecs-interface container
    emptyDir: # any other volume type is OK
      medium: "Memory"
```

ecs-interface container should also mount this volume, so a custom `volumeMount` is required, for example:
...
Note that `mountPath` variable is used to specify a location of trust store in ecs-interface container.
...
mTLS support can be added to ecs-interface application in two different ways.
...
```yaml
customVolumes:
  - name: key-store-volume-name # has to match name in initContainer and volumeMount in ecs-interface container
    emptyDir: # any other volume type is OK
      medium: "Memory"
```
...
Note that `mountPath` variable is used to specify a location of key store in ecs-interface container.
Suggested location is: `/mnt/k8s/trust-store`.
...
Besides required attributes, installation of ecs-interface can be customized in different ways.

### Multi-member setup

ecs-interface  application (along with all other HolisticPay applications) supports multi-member setup. In order to complete application setup, at least a mandatory set of attributes has to be defined:
...
ecs-interface application can use oAuth2 service for authorization. By default, this option is disabled, but can easily be enabled by specifying following attributes in values:
...
Custom environment variables can be added to ecs-interface container by applying `customEnv` value, for example:
...
Values file can be used to specify additional custom `volume` and `volumeMounts` to be added to ecs-interface container.

For example, custom volume mount could be added by defining this setup:

```yaml
customVolumes:
  - name: my-custom-volume # has to match name in initContainer and volumeMount in ecs-interface container
    emptyDir: # any other volume type is OK
      medium: "Memory"
...
ecs-interface application is predefined to redirect all logs to `stdout` expect for Web Server logs (`access.log`) and health check logs, which are not logged by default.
...
Default deployment strategy for ecs-interface application is `RollingUpdate`, but it can be overridden, along with other deployment parameters using following attributes (default values are shown):
...
By default, one replica of ecs-interface is installed on Kubernetes cluster. Number of replicas can be statically modified with above configuration, or `HorizontalPodAutoscaler` option can be used to let Kubernetes automatically scale application when required.

#### Customizing pod resource requests and limits

Following are the default values for ecs-interface requests and limits:
...
ecs-interface application has predefined health check probes (readiness and liveness).
...
Note that  ecs-interface  has health checks available within the `/health` endpoint (`/health/readiness` for readiness and `/health/liveness` for liveness), and this base paths should not modified, only query parameters are subject to change.
...
Security context for ecs-interface can be set on pod and/or on container level.
...
Note that container level security context will be applied to both containers in ecs-interface pod (Liquibase init container and ecs-interface container).

### Customizing network setup

#### Service setup

When installing ecs-interface using default setup, a `Service` object will be created of `ClusterIP` type exposed on port 8443.
...
ecs-interface deployment has some predefined affinity rules, as listed below:
...
There are some other customizable attributes predefined in  ecs-interface  application.
...
Finally, since  ecs-interface  is an Java application, there's a possibility to set custom JVM parameters.
