# sirius-specific application

## Purpose

This Helm chart installs sirius-specific application into your Kubernetes cluster.
...
It is not possible to install application using default values only, there is a list of required attributes which should be applied when installing sirius-specific.
...
`helm upgrade --install sirius-specific holisticpay/sirius-specific -f my-values.yaml` or

`helm upgrade --install sirius-specific holisticpay/sirius-specific --set required-key1=value1 --set required-key2=value2 ...`
...
sirius-specific application relies on Kafka and PostgreSQL backends.
...
sirius-specific (as well as all other HolisticPay applications) is a multi-member application. For this reason, at least one application member has to be defined in `members` structure for complete setup. Please refer to [Multi-member setup](#multi-member-setup) for details.
...
sirius-specific  uses Kafka as event stream backend.
Other than Kafka cluster itself,  sirius-specific e application also uses Kafka Schema Registry, which setup also has to be provided in order to establish required connection.
...
Default Kafka cluster and Kafka Schema registry connection type used by sirius-specific is Basic auth (username and password).
...
Kafka topics and consumer group names used by sirius-specific have default names defined in `values.yaml` file, but can be overridden with following setup:
...
image:
  ...
  app:
    registry: custom.app.image.registry # will override image.registry for sirius-specific app
  liquibase:
...
image:
  ...
  app:
    pullPolicy: Never # will override image.pullPolicy for sirius-specific image
  liquibase:
    pullPolicy: IfNotPresent # will override image.pullPolicy for Liquibase image
```

sirius-specific  image tag is normally read from Chart definition, but if required, it can be overridden with attribute `image.app.tag`, for example:
...
sirius-specific  image is located on Vestigo's private Docker Hub registry, and if image registry is set to docker.io, pull secret has to be defined.
Pull secret is not set by default, and it should be created prior to sirius-specific installation in target namespace.
...
sirius-specific  application is prepared to use TLS, but requires provided server certificate.
...
**Note that this secret has to be created in target namespace prior to installation of sirius-specific application.**
...
```yaml
customVolumes:
  - name: trust-store-volume-name # has to match name in initContainer and volumeMount in sirius-specific container
    emptyDir: # any other volume type is OK
      medium: "Memory"
```

sirius-specific container should also mount this volume, so a custom `volumeMount` is required, for example:
...
Note that `mountPath` variable is used to specify a location of trust store in sirius-specific container.
...
mTLS support can be added to sirius-specific application in two different ways.
...
```yaml
customVolumes:
  - name: key-store-volume-name # has to match name in initContainer and volumeMount in sirius-specific container
    emptyDir: # any other volume type is OK
      medium: "Memory"
```
...
Note that `mountPath` variable is used to specify a location of key store in sirius-specific container.
Suggested location is: `/mnt/k8s/trust-store`.
...
Besides required attributes, installation of sirius-specific can be customized in different ways.

### Multi-member setup

sirius-specific  application (along with all other HolisticPay applications) supports multi-member setup. In order to complete application setup, at least a mandatory set of attributes has to be defined:
...
Custom environment variables can be added to sirius-specific container by applying `customEnv` value, for example:
...
Values file can be used to specify additional custom `volume` and `volumeMounts` to be added to sirius-specific container.

For example, custom volume mount could be added by defining this setup:

```yaml
customVolumes:
  - name: my-custom-volume # has to match name in initContainer and volumeMount in sirius-specific container
    emptyDir: # any other volume type is OK
      medium: "Memory"
...
sirius-specific application is predefined to redirect all logs to `stdout` expect for Web Server logs (`access.log`) and health check logs, which are not logged by default.
...
Default deployment strategy for sirius-specific application is `RollingUpdate`, but it can be overridden, along with other deployment parameters using following attributes (default values are shown):
...
By default, one replica of sirius-specific is installed on Kubernetes cluster. Number of replicas can be statically modified with above configuration, or `HorizontalPodAutoscaler` option can be used to let Kubernetes automatically scale application when required.
...
Following are the default values for sirius-specific requests and limits:
...
CPU and/or memory utilization metrics can be used to autoscale sirius-specific pod.
...
sirius-specific application has predefined health check probes (readiness and liveness).
...
Note that  sirius-specific  has health checks available within the `/health` endpoint (`/health/readiness` for readiness and `/health/liveness` for liveness), and this base paths should not modified, only query parameters are subject to change.
...
Security context for sirius-specific can be set on pod and/or on container level.
...
Note that container level security context will be applied to both containers in sirius-specific pod (Liquibase init container and sirius-specific container).

### Customizing network setup

#### Service setup

When installing sirius-specific using default setup, a `Service` object will be created of `ClusterIP` type exposed on port 8443.
...
ingress:
  ...
  hosts:
    - host: sirius-specific.custom.url
      paths:
        - path: /
          pathType: Prefix
```
...
sirius-specific deployment has some predefined affinity rules, as listed below:
...
There are some other customizable attributes predefined in  sirius-specific  application.
...
Finally, since  sirius-specific  is an Java application, there's a possibility to set custom JVM parameters.
