...
- name: liquibase-{{ .memberSign | lower }}
  ...
  {{- else }}
  image: {{ printf "%s%s%s%s%s" (include "consumer-finance.liquibase.image" $) "-" ($member.memberSign  | lower ) ":" $.Values.image.liquibase.tag }}
  {{- end }}
...
Kafka login module
*/}}
{{- define "consumer-finance.kafka.loginModule" -}}
