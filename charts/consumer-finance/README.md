# consumer-finance-ms application


This Helm chart installs consumer-finance-ms application into your Kubernetes cluster.
...
It is not possible to install application using default values only, there is a list of required attributes which should be applied when installing consumer-finance-ms.
...
`helm upgrade --install consumer-finance-ms holisticpay/consumer-finance-ms -f my-values.yaml` or

`helm upgrade --install consumer-finance-ms holisticpay/consumer-finance-ms --set required-key1=value1 --set required-key2=value2 ...`
...
consumer-finance-ms application relies on Kafka and PostgreSQL backends.
...
consumer-finance-ms (as well as all other HolisticPay applications) is a multi-member application. For this reason, at least one application member has to be defined in `members` structure for complete setup. Please refer to [Multi-member setup](#multi-member-setup) for details.
...
consumer-finance-ms  uses Kafka as event stream backend.
Other than Kafka cluster itself,  consumer-finance-ms e application also uses Kafka Schema Registry, which setup also has to be provided in order to establish required connection.
...
Default Kafka cluster and Kafka Schema registry connection type used by consumer-finance-ms is Basic auth (username and password).
...
Kafka topics and consumer group names used by consumer-finance-ms have default names defined in `values.yaml` file, but can be overridden with following setup:
...
By default,  consumer-finance-ms  image is pulled directly from Vestigo's repository hosted by Docker Hub.
...
image:
  ...
  app:
    pullPolicy: Never # will override image.pullPolicy for consumer-finance-ms image
  liquibase:
    pullPolicy: IfNotPresent # will override image.pullPolicy for Liquibase image
```

consumer-finance-ms  image tag is normally read from Chart definition, but if required, it can be overridden with attribute `image.app.tag`, for example:
...
consumer-finance-ms  image is located on Vestigo's private Docker Hub registry, and if image registry is set to docker.io, pull secret has to be defined.
Pull secret is not set by default, and it should be created prior to consumer-finance-ms installation in target namespace.
...
consumer-finance-ms  application is prepared to use TLS, but requires provided server certificate.
...
**Note that this secret has to be created in target namespace prior to installation of consumer-finance-ms application.**
...
```yaml
customVolumes:
  - name: trust-store-volume-name # has to match name in initContainer and volumeMount in consumer-finance-ms container
    emptyDir: # any other volume type is OK
      medium: "Memory"
```

consumer-finance-ms container should also mount this volume, so a custom `volumeMount` is required, for example:
...
Note that `mountPath` variable is used to specify a location of trust store in consumer-finance-ms container.
...
mTLS support can be added to consumer-finance-ms application in two different ways.
...
```yaml
customVolumes:
  - name: key-store-volume-name # has to match name in initContainer and volumeMount in consumer-finance-ms container
    emptyDir: # any other volume type is OK
      medium: "Memory"
```
...
Note that `mountPath` variable is used to specify a location of key store in consumer-finance-ms container.
Suggested location is: `/mnt/k8s/trust-store`.
...
Besides required attributes, installation of consumer-finance-ms can be customized in different ways.


consumer-finance-ms  application (along with all other HolisticPay applications) supports multi-member setup. In order to complete application setup, at least a mandatory set of attributes has to be defined:
...
consumer-finance-ms application can use oAuth2 service for authorization. By default, this option is disabled, but can easily be enabled by specifying following attributes in values:
...
Custom environment variables can be added to consumer-finance-ms container by applying `customEnv` value, for example:
...
Values file can be used to specify additional custom `volume` and `volumeMounts` to be added to consumer-finance-ms container.

For example, custom volume mount could be added by defining this setup:

```yaml
customVolumes:
  - name: my-custom-volume # has to match name in initContainer and volumeMount in consumer-finance-ms container
    emptyDir: # any other volume type is OK
      medium: "Memory"
...
consumer-finance-ms application is predefined to redirect all logs to `stdout` expect for Web Server logs (`access.log`) and health check logs, which are not logged by default.
...
Default deployment strategy for consumer-finance-ms application is `RollingUpdate`, but it can be overridden, along with other deployment parameters using following attributes (default values are shown):
...
By default, one replica of consumer-finance-ms is installed on Kubernetes cluster. Number of replicas can be statically modified with above configuration, or `HorizontalPodAutoscaler` option can be used to let Kubernetes automatically scale application when required.


Following are the default values for consumer-finance-ms requests and limits:
...
CPU and/or memory utilization metrics can be used to autoscale consumer-finance-ms pod.
...
consumer-finance-ms application has predefined health check probes (readiness and liveness).
...
Note that  consumer-finance-ms  has health checks available within the `/health` endpoint (`/health/readiness` for readiness and `/health/liveness` for liveness), and this base paths should not modified, only query parameters are subject to change.
`scheme` attribute should also be set to `HTTPS` at all times, as well as `http` value for `port` attribute.


Security context for consumer-finance-ms can be set on pod and/or on container level.
...
Note that container level security context will be applied to both containers in consumer-finance-ms pod (Liquibase init container and consumer-finance-ms container).



When installing consumer-finance-ms using default setup, a `Service` object will be created of `ClusterIP` type exposed on port 8443.
...
ingress:
  ...
  hosts:
    - host: consumer-finance-ms.custom.url
      paths:
        - path: /
          pathType: Prefix
```
...
consumer-finance-ms deployment has some predefined affinity rules, as listed below:
...
There are some other customizable attributes predefined in  consumer-finance-ms  application.
...
Finally, since  consumer-finance-ms  is an Java application, there's a possibility to set custom JVM parameters.
...
This configuration is used for setting up the connection to the gRPC server which provides functionalities for getting person structure data.
...
### Maximum transaction number in CF eligibility check request

This configuration represents maximum number of transactions in the single request for CF eligibility check.
Default value is 10.

```yaml
trx:
  max:
    number: 10 # default value is 10
```

### Caching custom configuration

In Consumer Finance application, parametrization that is often used is cached or temporarily stored in the memory for the
