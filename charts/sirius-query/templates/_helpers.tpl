...
{{/*
sirius-query image repository
*/}}
{{- define "sirius-query.app.repository" -}}
{{- if .Values.image.app.imageLocation }}
{{- .Values.image.app.imageLocation }}
{{- else }}
{{- $psRepo := "hrvestigo/sirius-query-ms" }}
...
{{/*
sirius-query image pull policy
*/}}
{{- define "sirius-query.app.imagePullPolicy" -}}
...
*/}}
{{- define "sirius-query.liquibase.initContainer" }}
{{- range $k, $member := .Values.members }}
- name: liquibase-{{ .memberSign | lower }}
  securityContext:
  {{- toYaml $.Values.securityContext | nindent 4 }}
  image: {{ printf "%s%s%s%s%s" (include "sirius-query.liquibase.image" $) "-" ($member.businessUnit | lower ) ":" $.Values.image.liquibase.tag }}
  imagePullPolicy: {{ default "IfNotPresent" (default $.Values.image.pullPolicy $.Values.image.liquibase.pullPolicy) }}
  resources:
    {{- include "sirius-query.liquibase.initContainer.resources" $ | nindent 4 }}
  volumeMounts:
    - mountPath: /liquibase/secret/
      name: {{ include "sirius-query.name" $ }}-secret
  env:
    - name: SCHEMA_NAME
  ...
  command:
    ...
    - -c
  {{- if $member.datasource }}
    {{- $url := printf "%s%s%s%d%s%s" "jdbc:db2://" (default $.Values.datasource.host $member.datasource.host) ":" (default ($.Values.datasource.port | int) ( $member.datasource.port | int)) "/" (default $.Values.datasource.dbName  $member.datasource.dbName) }}
    ...
    {{- end }}
  {{- else }}
    {{- $url := printf "%s%s%s%d%s%s" "jdbc:db2://" $.Values.datasource.host ":" ($.Values.datasource.port | int) "/" $.Values.datasource.dbName }}
    {{- $context := printf "%s%s%s" ( required "Please specify business unit in members.businessUnit" $member.businessUnit | upper ) ( required "Please specify application member in members.applicationMember" $member.applicationMember | upper ) ",test" }}
    {{- $params := printf "%s%s%s%s%s%s" "cp /liquibase/changelog/liquibase.properties /tmp && java -jar /tmp/aesdecryptor.jar -d -l && /liquibase/docker-entrypoint.sh --defaultsFile=/tmp/liquibase.properties --url=" $url " --contexts=" $context " --username=" $.Values.liquibase.user }}
    {{- if $.Values.liquibase.syncOnly }}
    - {{ printf "%s%s" $params " changelog-sync" }}
...
*/}}
{{- define "sirius-query.liquibase.initContainer.resources" -}}
...
*/}}
{{- define "sirius-query.trustStoreEnv" -}}
...
Common labels
*/}}
{{- define "sirius-query.labels" -}}
helm.sh/chart: {{ include "sirius-query.chart" . }}
app: {{ include "sirius-query.name" . }}
project: HolisticPay
{{ include "sirius-query.selectorLabels" . }}
{{- if .Chart.AppVersion }}
...
Selector labels
*/}}
{{- define "sirius-query.selectorLabels" -}}
app.kubernetes.io/name: {{ include "sirius-query.name" . }}
...
- name: {{ include "sirius-query.name" . }}-secret
  secret:
    secretName: {{ include "sirius-query.name" . }}-secret
    items:
      - path: password.conf
        key: password.conf
- name: {{ include "sirius-query.name" . }}-configmap
  configMap:
    name: {{ include "sirius-query.name" . }}-configmap
- name: server-cert
...
{{/*
Mounts for sirius-query application
*/}}
{{- define "sirius-query.mounts" -}}
...
*/}}
{{- define "sirius-query.db.connectionParams" -}}
{{- $atts := list -}}
...
Kafka login module
*/}}
{{- define "sirius-query.kafka.loginModule" -}}
