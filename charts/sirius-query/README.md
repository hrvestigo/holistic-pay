# sirius-query application

## Purpose

This Helm chart installs sirius-query application into your Kubernetes cluster.
...
It is not possible to install application using default values only, there is a list of required attributes which should be applied when installing sirius-query.
...
`helm upgrade --install sirius-query holisticpay/sirius-query -f my-values.yaml` or

`helm upgrade --install sirius-query holisticpay/sirius-query --set required-key1=value1 --set required-key2=value2 ...`
...
sirius-query application relies on Kafka and PostgreSQL backends.
...
sirius-query (as well as all other HolisticPay applications) is a multi-member application. For this reason, at least one application member has to be defined in `members` structure for complete setup. Please refer to [Multi-member setup](#multi-member-setup) for details.
...
sirius-query  uses Kafka as event stream backend.
Other than Kafka cluster itself,  sirius-query e application also uses Kafka Schema Registry, which setup also has to be provided in order to establish required connection.
...
Default Kafka cluster and Kafka Schema registry connection type used by sirius-query is Basic auth (username and password).
...
Kafka topics and consumer group names used by sirius-query have default names defined in `values.yaml` file, but can be overridden with following setup:
...
By default,  sirius-query  image is pulled directly from Vestigo's repository hosted by Docker Hub.
...
image:
  ...
  app:
    pullPolicy: Never # will override image.pullPolicy for sirius-query image
  liquibase:
    pullPolicy: IfNotPresent # will override image.pullPolicy for Liquibase image
```

sirius-query  image tag is normally read from Chart definition, but if required, it can be overridden with attribute `image.app.tag`, for example:
...
sirius-query  image is located on Vestigo's private Docker Hub registry, and if image registry is set to docker.io, pull secret has to be defined.
Pull secret is not set by default, and it should be created prior to sirius-query installation in target namespace.
...
sirius-query  application is prepared to use TLS, but requires provided server certificate.
...
**Note that this secret has to be created in target namespace prior to installation of sirius-query application.**
...
```yaml
customVolumes:
  - name: trust-store-volume-name # has to match name in initContainer and volumeMount in sirius-query container
    emptyDir: # any other volume type is OK
      medium: "Memory"
```

sirius-query container should also mount this volume, so a custom `volumeMount` is required, for example:
...
Note that `mountPath` variable is used to specify a location of trust store in sirius-query container.
...
mTLS support can be added to sirius-query application in two different ways.
...
```yaml
customVolumes:
  - name: key-store-volume-name # has to match name in initContainer and volumeMount in sirius-query container
    emptyDir: # any other volume type is OK
      medium: "Memory"
```
...
Note that `mountPath` variable is used to specify a location of key store in sirius-query container.
Suggested location is: `/mnt/k8s/trust-store`.
...
Besides required attributes, installation of sirius-query can be customized in different ways.

### Multi-member setup

sirius-query  application (along with all other HolisticPay applications) supports multi-member setup. In order to complete application setup, at least a mandatory set of attributes has to be defined:
...
sirius-query application can use oAuth2 service for authorization. By default, this option is disabled, but can easily be enabled by specifying following attributes in values:
...
Custom environment variables can be added to sirius-query container by applying `customEnv` value, for example:
...
Values file can be used to specify additional custom `volume` and `volumeMounts` to be added to sirius-query container.

For example, custom volume mount could be added by defining this setup:

```yaml
customVolumes:
  - name: my-custom-volume # has to match name in initContainer and volumeMount in sirius-query container
    emptyDir: # any other volume type is OK
      medium: "Memory"
...
sirius-query application is predefined to redirect all logs to `stdout` expect for Web Server logs (`access.log`) and health check logs, which are not logged by default.
...
Default deployment strategy for sirius-query application is `RollingUpdate`, but it can be overridden, along with other deployment parameters using following attributes (default values are shown):
...
By default, one replica of sirius-query is installed on Kubernetes cluster. Number of replicas can be statically modified with above configuration, or `HorizontalPodAutoscaler` option can be used to let Kubernetes automatically scale application when required.

#### Customizing pod resource requests and limits

Following are the default values for sirius-query requests and limits:
...
CPU and/or memory utilization metrics can be used to autoscale sirius-query pod.
...
sirius-query application has predefined health check probes (readiness, liveness and startup).
...
Note that  sirius-query  has health checks available within the `/health` endpoint (`/health/readiness` for readiness and `/health/liveness` for liveness), and this base paths should not modified, only query parameters are subject to change.
...
Security context for sirius-query can be set on pod and/or on container level.
...
Note that container level security context will be applied to both containers in sirius-query pod (Liquibase init container and sirius-query container).

### Customizing network setup

#### Service setup

When installing sirius-query using default setup, a `Service` object will be created of `ClusterIP` type exposed on port 8443.
...
sirius-query deployment has some predefined affinity rules, as listed below:
...
There are some other customizable attributes predefined in  sirius-query  application.
...
Finally, since  sirius-query  is an Java application, there's a possibility to set custom JVM parameters.
