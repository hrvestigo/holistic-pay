...
`helm upgrade --install person-registry-web holisticpay/person-registry-web -f my-values.yaml` or

`helm upgrade --install person-registry-web holisticpay/person-registry-web --set required-key1=value1 --set required-key2=value2 ...`
...
Custom environment variables can be added to person-registry-web container by applying `customEnv` value, for example:
...
Values file can be used to specify additional custom `volume` and `volumeMounts` to be added to person-registry-web container.

For example, custom volume mount could be added by defining this setup:

```yaml
customVolumes:
  - name: my-custom-volume # has to match name in initContainer and volumeMount in person-registry-web container
...
ingress:
  ...
  hosts:
    - host: person-registry-web.custom.url
      paths:
        - path: /
          pathType: Prefix
```
