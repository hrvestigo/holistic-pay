# balance-log application

## Purpose

This Helm chart installs balance-log application into your Kubernetes cluster.
...
It is not possible to install application using default values only, there is a list of required attributes which should be applied when installing balance-log.
...
`helm upgrade --install balance-log holisticpay/balance-log -f my-values.yaml` or

`helm upgrade --install balance-log holisticpay/balance-log --set required-key1=value1 --set required-key2=value2 ...`
...
balance-log application relies on Kafka and PostgreSQL backends.
...
balance-log (as well as all other HolisticPay applications) is a multi-member application. For this reason, at least one application member has to be defined in `members` structure for complete setup. Please refer to [Multi-member setup](#multi-member-setup) for details.
...
Other than Kafka cluster itself,  balance-log e application also uses Kafka Schema Registry, which setup also has to be provided in order to establish required connection.
...
Default Kafka cluster and Kafka Schema registry connection type used by balance-log is Basic auth (username and password).
...
Kafka topics and consumer group names used by balance-log have default names defined in `values.yaml` file, but can be overridden with following setup:
...
original topic name = hr.vestigo.hp.balanceaccountbucket
retry topic name = hr.vestigo.hp.balanceaccountbucketretry
dead letter topic name = hr.vestigo.hp.balanceaccountbucketdlt

# topic name with version
original topic name = hr.vestigo.hp.balanceaccountbucket.01
retry topic name = hr.vestigo.hp.balanceaccountbucketretry.01
dead letter topic name = hr.vestigo.hp.balanceaccountbucketdlt.01
```
...
By default,  balance-log  image is pulled directly from Vestigo's repository hosted by Docker Hub.
...
image:
  ...
  app:
    pullPolicy: Never # will override image.pullPolicy for balance-log image
  liquibase:
    pullPolicy: IfNotPresent # will override image.pullPolicy for Liquibase image
```

balance-log  image tag is normally read from Chart definition, but if required, it can be overridden with attribute `image.app.tag`, for example:
...
balance-log  image is located on Vestigo's private Docker Hub registry, and if image registry is set to docker.io, pull secret has to be defined.
Pull secret is not set by default, and it should be created prior to balance-log installation in target namespace.
...
balance-log  application is prepared to use TLS, but requires provided server certificate.
...
**Note that this secret has to be created in target namespace prior to installation of balance-log application.**
...
```yaml
customVolumes:
  - name: trust-store-volume-name # has to match name in initContainer and volumeMount in balance-log container
    emptyDir: # any other volume type is OK
      medium: "Memory"
```

balance-log container should also mount this volume, so a custom `volumeMount` is required, for example:
...
Note that `mountPath` variable is used to specify a location of trust store in balance-log container.
...
mTLS support can be added to balance-log application in two different ways.
...
```yaml
customVolumes:
  - name: key-store-volume-name # has to match name in initContainer and volumeMount in balance-log container
    emptyDir: # any other volume type is OK
      medium: "Memory"
```
...
Note that `mountPath` variable is used to specify a location of key store in balance-log container.
Suggested location is: `/mnt/k8s/trust-store`.
...
Besides required attributes, installation of balance-log can be customized in different ways.

### Multi-member setup

balance-log  application (along with all other HolisticPay applications) supports multi-member setup. In order to complete application setup, at least a mandatory set of attributes has to be defined:
...
balance-log application can use oAuth2 service for authorization. By default, this option is disabled, but can easily be enabled by specifying following attributes in values:
...
Custom environment variables can be added to balance-log container by applying `customEnv` value, for example:
...
Values file can be used to specify additional custom `volume` and `volumeMounts` to be added to balance-log container.

For example, custom volume mount could be added by defining this setup:

```yaml
customVolumes:
  - name: my-custom-volume # has to match name in initContainer and volumeMount in balance-log container
    emptyDir: # any other volume type is OK
      medium: "Memory"
...
balance-log application is predefined to redirect all logs to `stdout` expect for Web Server logs (`access.log`) and health check logs, which are not logged by default.
...
Default deployment strategy for balance-log application is `RollingUpdate`, but it can be overridden, along with other deployment parameters using following attributes (default values are shown):
...
By default, one replica of balance-log is installed on Kubernetes cluster. Number of replicas can be statically modified with above configuration, or `HorizontalPodAutoscaler` option can be used to let Kubernetes automatically scale application when required.

#### Customizing pod resource requests and limits

Following are the default values for balance-log requests and limits:
...
CPU and/or memory utilization metrics can be used to autoscale balance-log pod.
...
balance-log application has predefined health check probes (readiness,liveness and startup).
...
Note that  balance-log  has health checks available within the `/health` endpoint (`/health/readiness` for readiness and `/health/liveness` for liveness), and this base paths should not modified, only query parameters are subject to change.
...
Security context for balance-log can be set on pod and/or on container level.
...
Note that container level security context will be applied to both containers in balance-log pod (Liquibase init container and balance-log container).

### Customizing network setup

#### Service setup

When installing balance-log using default setup, a `Service` object will be created of `ClusterIP` type exposed on port 8443.
...
balance-log deployment has some predefined affinity rules, as listed below:
...
There are some other customizable attributes predefined in  balance-log  application.
...
Finally, since  balance-log  is an Java application, there's a possibility to set custom JVM parameters.
