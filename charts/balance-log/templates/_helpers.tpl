...
{{/*
balance-log image repository
*/}}
{{- define "balance-log.app.repository" -}}
{{- if .Values.image.app.imageLocation }}
{{- .Values.image.app.imageLocation }}
{{- else }}
{{- $psRepo := "hrvestigo/balance-log-ms" }}
...
{{/*
balance-log image pull policy
*/}}
{{- define "balance-log.app.imagePullPolicy" -}}
...
*/}}
{{- define "balance-log.liquibase.initContainer" }}
{{- range $k, $member := .Values.members }}
- name: liquibase-{{ .memberSign | lower }}
  securityContext:
  {{- toYaml $.Values.securityContext | nindent 4 }}
  {{- if $.Values.image.liquibase.imageLocation }}
  image: {{ include "balance-log.liquibase.image" $ }}
  {{- else }}
  image: {{ printf "%s%s%s%s%s" (include "balance-log.liquibase.image" $) "-" ($member.businessUnit | lower ) ":" $.Values.image.liquibase.tag }}
  {{- end }}
  imagePullPolicy: {{ default "IfNotPresent" (default $.Values.image.pullPolicy $.Values.image.liquibase.pullPolicy) }}
  resources:
    {{- include "balance-log.liquibase.initContainer.resources" $ | nindent 4 }}
  volumeMounts:
    - mountPath: /liquibase/secret/
      name: {{ include "balance-log.name" $ }}-secret
  env:
    - name: SCHEMA_NAME
  {{- if $member.datasource }}
    {{- if $member.datasource.globalSchema }}
      value: {{ $member.businessUnit | lower }}{{ required "Please specify global schema prefix in datasource.globalSchemaPrefix" $.Values.datasource.globalSchemaPrefix }}{{ include "balance-log.dbSchema" $ }}{{ required "Please specify environment label in env.label" $.Values.env.label | lower }}
    {{- else }}
      value: {{ $member.businessUnit | lower }}{{ $member.applicationMember | lower }}{{ include "balance-log.dbSchema" $ }}{{ required "Please specify environment label in env.label" $.Values.env.label | lower }}
    {{- end }}
  {{- else }}
      value: {{ $member.businessUnit | lower }}{{ $member.applicationMember | lower }}{{ include "balance-log.dbSchema" $ }}{{ required "Please specify environment label in env.label" $.Values.env.label | lower }}
  {{- end }}
  ...
  {{- if $member.datasource }}
    ...
    {{- $context := printf "%s%s%s%s%s" ( required "Please specify business unit in members.businessUnit" $member.businessUnit | upper ) ( required "Please specify application member in members.applicationMember" $member.applicationMember | upper ) ",test" (ternary ",outbox" "" $.Values.application.database.outbox.enabled)
      (ternary ",partitions" "" $.Values.application.database.partitions.enabled) }}
...
*/}}
{{- define "balance-log.liquibase.initContainer.resources" -}}
...
*/}}
{{- define "balance-log.trustStoreEnv" -}}
...
Common labels
*/}}
{{- define "balance-log.labels" -}}
helm.sh/chart: {{ include "balance-log.chart" . }}
app: {{ include "balance-log.name" . }}
project: HolisticPay
{{ include "balance-log.selectorLabels" . }}
{{- if .Chart.AppVersion }}
...
Selector labels
*/}}
{{- define "balance-log.selectorLabels" -}}
app.kubernetes.io/name: {{ include "balance-log.name" . }}
...
- name: {{ include "balance-log.name" . }}-secret
  secret:
    secretName: {{ include "balance-log.name" . }}-secret
    items:
      - path: password.conf
        key: password.conf
- name: {{ include "balance-log.name" . }}-configmap
  configMap:
    name: {{ include "balance-log.name" . }}-configmap
- name: server-cert
...
{{/*
Mounts for balance-log application
*/}}
{{- define "balance-log.mounts" -}}
...
*/}}
{{- define "balance-log.db.connectionParams" -}}
{{- $atts := list -}}
...
{{/*
Balance log Kafka topics configuration
*/}}
{{- define "balance-log.kafka.topics" }}
{{- range $key, $value := .Values.kafka.topics }}
{{- range $k, $v := $value }}
- name: BALANCE_LOG_KAFKA_TOPICS_{{ $key | upper }}_{{ $k | snakecase | upper }}
  value: {{ $v | quote }}
{{- end }}
{{- end }}
{{- end }}
