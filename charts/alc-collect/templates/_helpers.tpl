...
{{- else }}
{{- $liquiRepo := "hrvestigo/alc-collect-lb" }}
...
- name: liquibase-{{ .memberSign | lower }}
  ...
  {{- else }}
  image: {{ printf "%s%s%s%s%s" (include "alc-collect.liquibase.image" $) "-" ($member.memberSign | lower ) ":" $.Values.image.liquibase.tag }}
  {{- end }}
...
*/}}
{{- define "alc-collect.liquibase.initContainer.resources" -}}
...
*/}}
{{- define "alc-collect.db.connectionParams" -}}
{{- $atts := list -}}
