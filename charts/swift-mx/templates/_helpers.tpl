...
- name: liquibase-{{ .memberSign | lower }}
  ...
  {{- else }}
  image: {{ printf "%s%s%s%s%s" (include "swift-mx.liquibase.image" $) "-" ($member.memberSign | lower ) ":" $.Values.image.liquibase.tag }}
  {{- end }}
  ...
  {{- if $member.datasource }}
    ...
    {{- $context := printf "%s%s%s%s%s%s" ( required "Please specify business unit in members.businessUnit" $member.businessUnit | upper ) ( required "Please specify application member in members.applicationMember" $member.applicationMember | upper ) ",test" (ternary ",event-log" "" $.Values.application.data.eventsLog.enabled) (ternary ",partitions" "" $.Values.application.data.eventsLog.partitionsEnabled) (ternary ",file-int" "" (or $.Values.application.integration.file.readEnabled $.Values.application.integration.file.writeEnabled)) }}
  ...
  {{- else }}
    ...
    {{- $context := printf "%s%s%s%s%s%s" ( required "Please specify business unit in members.businessUnit" $member.businessUnit | upper ) ( required "Please specify application member in members.applicationMember" $member.applicationMember | upper ) ",test" (ternary ",event-log" "" $.Values.application.data.eventsLog.enabled) (ternary ",partitions" "" $.Values.application.database.eventsLog.partitionsEnabled) (ternary ",file-int" "" (or $.Values.application.integration.file.readEnabled $.Values.application.integration.file.writeEnabled)) }}
    {{- $params := printf "%s%s%s%s%s%s" "cp /liquibase/changelog/liquibase.properties /tmp && java -jar /tmp/aesdecryptor.jar -d -l && /liquibase/docker-entrypoint.sh --defaultsFile=/tmp/liquibase.properties --url=" $url " --contexts=" $context " --username=" $.Values.liquibase.user }}
    {{- if $.Values.liquibase.syncOnly }}
    - {{ printf "%s%s" $params " changelog-sync" }}
    {{- else }}
...
Kafka login module
*/}}
{{- define "swift-mx.kafka.loginModule" -}}
